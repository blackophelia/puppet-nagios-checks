...
# at IX and has kindly been made available to the Open Source community for #
...
def status():
    ...
    if node_status_totals[FAILED] + node_status_totals[UNRESPONSIVE] + node_status_totals[UNREPORTED]  >=  options.critical_threshold:
        status = 'CRITICAL:'
        exit = CRITICAL
    elif node_status_totals[FAILED] > 0 and options.critical_on_failed:
        status = 'CRITICAL:'
        exit = CRITICAL
...
if __name__ == '__main__':
    ...
    parser.add_option("-d", "--detailed", action="store_true", dest="detailed", default=True, help="Print details - includes host details.  Default.")
    parser.add_option("-w", "--warning", action="store", dest="warning_threshold", type="int", default="1", help="Warning if <num> hosts are failing/unresponsive/unreported.  Defaults to 1.")
    parser.add_option("-c", "--critical", action="store", dest="critical_threshold", type="int", default="4", help="Critical if <num> hosts are failing/unresponsive/unreported.  Defaults to 4.")
    ...
    parser.add_option("-W", "--warn-failed", action="store_false", dest="critical_on_failed", help="Failed nodes won't trigger critical state.")
    parser.add_option("-C", "--crit-failed", action="store_true", dest="critical_on_failed", help="Failed nodes will trigger critical state. Default.")
